...
export default function Budget({ siteId }: BudgetProps) {
    ...
    const fetchBudget = async () => {
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:3002/montyhle-tire-budget/site/${siteId}`);
    ...
    const fetchBudgetByYear = async (year: number) => {
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:3002/montyhle-tire-budget/site/${siteId}/year/${year}`);
    ...
    const handleSaveAll = async () => {
        ...
        try {
            const updatePromises = Object.entries(editedBudgets).map(([budgetId, tireCountStr]) => {
                ...
                return axios.patch(`http://localhost:3002/montyhle-tire-budget/${budgetId}`, {
                    siteId,
